...
public class FeuilleDEmargementPersonnalise {


    public static ByteArrayOutputStream createDoc(List<List<Enregistrement>> listListEnregistrements, List<String> places) {
        ...
        document.open();
int decalage=0;
        for (int l = 0; l < listListEnregistrements.size(); l++) {
            Session session = listListEnregistrements.get(l).get(0).getIdSession();
            Niveau niveau = listListEnregistrements.get(l).get(0).getIdNiveau();
            Parcours parcours = niveau.getParcours();
            Specialite specialite = parcours.getSpecialite();
            Cycle cycle = specialite.getCycle();
            List<Personne> personneList = new ArrayList<>();
            for (Enregistrement e : listListEnregistrements.get(l)) {
            ...
            try {
                ...
                for (int page = 0; page < nbpage; page++) {
                    ...
                    for (int i = start; i <= n; i++) {
                        ...
                        t.addCell(cell2);
                        cell2 = new PdfPCell(new Phrase(places.get((i - 1)+decalage)));
                        cell2.setColspan(2);
                        cell2.setHorizontalAlignment(Element.ALIGN_CENTER);
                        cell2.setVerticalAlignment(Element.ALIGN_MIDDLE);
                        t.addCell(cell2);
            ...
            decalage += listListEnregistrements.get(l).size();
        }
        document.close();
        return response;
    }
